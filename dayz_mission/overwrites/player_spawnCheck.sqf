...

{
	...
	if(_canLoot) then {
		...
		//Loot
        //########### BEGIN MODIFIED CODE: LOOT RADIUS CHECK
		//if ((_dis < 120) and (_dis > 30) and !_inVehicle) then {
		if ((_dis < (DZ_LOOT_SPAWN_RANGE select 1)) and (_dis > (DZ_LOOT_SPAWN_RANGE select 0)) and (DZ_SPAWN_LOOT_IN_VEHICLE or !_inVehicle)) then {
        //########### BEGIN MODIFIED CODE: LOOT RADIUS CHECK
