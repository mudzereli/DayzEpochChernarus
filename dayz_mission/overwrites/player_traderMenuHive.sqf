...
TraderDialogLoadItemList = {
	...
	if(isNil "PVDZE_plr_TradeMenuResult") then {
        diag_log text "Loading Trade Menu Results...";
	...
	_item_list = [];
	{
        ...
        switch (true) do {
            case (_count > 0 && _qty > 0): {
                lbSetColor [TraderDialogItemList, _index, [.2, .8, .8, 1]];
            };
            case (_count > 0): {
                lbSetColor [TraderDialogItemList, _index, [.2, .8, .2, 1]];
            };
            case (_qty > 0): {
                lbSetColor [TraderDialogItemList, _index, [.2, .4, 1, 1]];
            };
            default {
                lbSetColor [TraderDialogItemList, _index, [1, 1, 1, 1]];
            };
        };
