...
switch (_iClass) do
{
	default
	{
		...
		_qty = 0;
        //### BEGIN MODIFIED CODE
		_max = 1 + round(random (2 * DZ_SPAWN_LOOT_MULTIPLIER * DZ_SPAWN_LOOT_DEFAULT_MULTIPLIER));
	...
	case "cfglootweapon":
	{
		...
		if ((count _mags) > 0) then
		{
            ...
            //### BEGIN MODIFIED CODE
			_item addMagazineCargoGlobal [(_mags select 0), (round(random (2 * DZ_SPAWN_LOOT_MULTIPLIER * DZ_SPAWN_LOOT_WEAPON_MAG_MULTIPLIER)))];
	...
	case "supplies":
	{
		//Item is a stack of building supplies
		_item = createVehicle ["WeaponHolder", _iPos, [], _radius, "CAN_COLLIDE"];
		_item addMagazineCargoGlobal [_iItem,round(random (2 * DZ_SPAWN_LOOT_MULTIPLIER * DZ_SPAWN_LOOT_SUPPLIES_MULTIPLIER))];
	};
    //### END MODIFIED CODE
	case "object":
	{
